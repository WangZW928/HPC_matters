...
void VwisAmrExSolver::run_cartesian_benchmark(
    amrex::Real dt, int steps, amrex::Real viscosity, std::string const& report_path)
{
    ...
    if (amrex::ParallelDescriptor::IOProcessor()) {
        ...
        output << "{\n  \"schema\": \"vwis-cartesian-benchmark-v1\",\n"
               ...
               << "  \"amrex_locked_version\": \"" << VWIS_AMREX_LOCKED_VERSION << "\",\n"
               << "  \"dimension\": 3, \"n_cell\": [" << m_geom.Domain().length(0) << ", "
    ...
    auto write_metric = [&](int step, bool comma) {
        ...
        amrex::Real integrated_div = divergence.sum(0, true) * m_cell_volume;
    ...
    amrex::Print() << "VWiS Cartesian benchmark: PASS report=" << report_path
