...
char const* boundary_name(CartesianBC bc)
{
    switch (bc) {
...
void VwisAmrExSolver::validate_boundary_config() const
{
    ...
    if (m_nghost < 1) throw std::runtime_error("P3 physical BC requires vwisbcs.enabled=1 and vwisbcs.nghost>=1");
...
{
    amrex::MultiFab profile(m_ucont[dir].boxArray(), m_dm, 1, 0);
    profile.setVal(0.0);
    const amrex::Box face_domain = amrex::convert(m_geom.Domain(), amrex::IntVect::TheDimensionVector(dir));
    ...
    for (amrex::MFIter mfi(profile, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(mfi.validbox(), [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept {
            ...
            if (index[dir] == boundary_index) {
                ...
                value(i,j,k) = area * (offset + linear * (slope0 * x0 + slope1 * x1));
            }
        });
    }
    amrex::Real local_sum = profile.sum_unique(0, true, m_geom.periodicity());
    ...
    if (m_boundary.enabled) {
        for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
            amrex::Print() << "  boundary dir=" << dir << " lo="
                           << boundary_name(m_boundary.sides[2 * dir].velocity) << " hi="
                           << boundary_name(m_boundary.sides[2 * dir + 1].velocity) << "\n";
        }
    }
}
