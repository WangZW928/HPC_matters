...
void AVWiSCaseRunnerAccess::run_physical_channel_benchmark_impl(
    ...
    std::string const& report_path)
{
    ...
    if (solver.m_geom.isPeriodic(0) || !solver.m_geom.isPeriodic(2) || !solver.m_boundary.enabled ||
        solver.m_boundary.sides[0].velocity != CartesianBC::Inflow ||
        solver.m_boundary.sides[1].velocity != CartesianBC::Outflow ||
        solver.m_boundary.sides[2].velocity != CartesianBC::NoSlipWall ||
        solver.m_boundary.sides[3].velocity != CartesianBC::NoSlipWall) {
    ...
    solver.m_ucat.setVal(0.0);
    for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
        solver.m_ucont[dir].setVal(0.0);
        solver.m_ucont_old[dir].setVal(0.0);
        solver.m_ucont_older[dir].setVal(0.0);
    }
    solver.mark_valid_modified();
    solver.apply_boundary_pipeline("physical-channel-initialize");
    ...
    if (amrex::ParallelDescriptor::IOProcessor()) {
        ...
        output << "{\n  \"schema\": \"avwis-physical-channel-v1\",\n"
               ...
               << "  \"domain\": [[0,0,0],[1,1,1]],\n"
               << "  \"n_cell\": [" << solver.m_geom.Domain().length(0) << ", "
               << solver.m_geom.Domain().length(1) << ", " << solver.m_geom.Domain().length(2) << "],\n"
               << "  \"dx\": [" << solver.m_dx[0] << ", " << solver.m_dx[1] << ", " << solver.m_dx[2] << "],\n"
               << "  \"dt\": " << dt << ",\n  \"nu\": " << viscosity
               << ",\n  \"Re_inlet\": " << (solver.m_boundary.inlet_target_flux / (solver.m_geom.ProbLength(1) * solver.m_geom.ProbLength(2))) / viscosity
               << ",\n  \"steps\": " << steps << ",\n  \"final_time\": " << dt * steps << ",\n"
    ...
    const amrex::Box& domain = solver.m_geom.Domain();
    const int xlo = domain.smallEnd(0);
    const int xhi = domain.bigEnd(0);
    const amrex::Real cross_section = solver.m_geom.ProbLength(1) * solver.m_geom.ProbLength(2);
    const amrex::Real inlet_speed = solver.m_boundary.inlet_target_flux / cross_section;
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        const UniformFlowDiagnostics flow = solver.uniform_flow_diagnostics();
        const UniformPlaneStatistics inlet_section = solver.uniform_plane_statistics(0, xlo);
        const UniformPlaneStatistics outlet_section = solver.uniform_plane_statistics(0, xhi);
        amrex::Array<amrex::Real, AMREX_SPACEDIM> center_position{};
        for (int dir = 0; dir < AMREX_SPACEDIM; ++dir) {
            const int center_index = domain.smallEnd(dir) + domain.length(dir) / 2;
            center_position[dir] = solver.m_geom.ProbLo(dir) +
                (static_cast<amrex::Real>(center_index - domain.smallEnd(dir)) + 0.5) * solver.m_dx[dir];
        }
        const UniformPointSample center = solver.sample_uniform_point(center_position);
        ...
        if (amrex::ParallelDescriptor::IOProcessor()) {
            if (step != 0) output << ",\n";
            output << "    {\"step\": " << step + 1 << ", \"time\": " << solver.m_time
    ...
    amrex::Print() << "AVWiS physical Cartesian channel run complete: report=" << report_path
                   << " steps=" << steps << " final_time=" << solver.m_time << "\n";
}

void run_physical_channel_benchmark(AVWiSSolver& solver, amrex::Real dt,
                                    int steps, amrex::Real viscosity,
                                    std::string const& report_path)
{
    AVWiSCaseRunnerAccess::run_physical_channel_benchmark_impl(
